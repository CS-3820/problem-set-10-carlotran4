...
smallStep (App (App (Lam s (Lam s1 (Var s2))) e1) e2,acc)
  | s == s2 && isThrow e1 = Just (App e1 e2,acc)
  | isThrow e2 = Just (App (Lam s1 e1) e2,acc)


smallStep (App (App (Lam x body) v1) v, acc) = Just (App (subst x v1 body) v, acc)
